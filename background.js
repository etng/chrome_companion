var rule_global = new RegExp(
  "(^|&)(utm_source|utm_medium|utm_term|utm_content|utm_campaign|utm_reader|utm_place|ga_source|ga_medium|ga_term|ga_content|ga_campaign|ga_place|yclid|_openstat|fb_action_ids|fb_action_types|fb_ref|fb_source|action_object_map|action_type_map|action_ref_map|_hsenc|_hsmi|hmsr)=[^&]*",
  "ig"
);
...
var clean_url = function(url) {
  ...
  if (rule_global !== null && qpos > -1) {
    ...
    cleared = cleared.replace(/(^|&)hmsr=[^&]*(&|$)/i, "");
    cleared = cleared.replace(/(^|&)utm_medium=[^&]*(&|$)/i, "");
    cleared = cleared.replace(/(^|&)utm_source=[^&]*(&|$)/i, "");
    cleared = cleared.replace(/(^|&)f=tt(&|$)/i, "");
    cleared = cleared.replace(/(^|&)fr=email(&|$)/i, "");
    // console.log('after', cleared)
    if (host === "youtube.com")
      cleared = cleared.replace(rule_youtube, "").replace(/^[&]+/i, "");
    if (host === "facebook.com")
      cleared = cleared.replace(rule_facebook, "").replace(/^[&]+/i, "");
    if (host === "imdb.com")
      cleared = cleared.replace(rule_imdb, "").replace(/^[&]+/i, "");
...
chrome.webRequest.onBeforeRequest.addListener(
  ...
  },
  {
    urls: [
    "*://pos.baidu.com/*",
    "*://g.csdnimg.cn/side-toolbar/*",
    "*://csdnimg.cn/public/common/indexSuperise/*",
...
// });
var content_scripts = [
  ["wx.qq.js", /^https?:\/\/mp\.weixin\.qq\.com\/s\/[^\/]*/g],
  // [
  // 	'vlive_tv.js',
  // 	/^https?:\/\/(channels|www)\.vlive\.tv\//g
  // ],
  ["segmentfault.js", /^https?:\/\/segmentfault\.com\/(a|p)\/\d+/g],
  ["zhihu.js", /^https?:\/\/zhuanlan\.zhihu\.com\/p\/\d+(\?|$)/g],
  [
    "medium.js",
    [
  ...
  ["infoq.js", /^https?:\/\/www\.infoq\.com\/cn\/articles\/.*$/g],
  ["ruanyifeng.js", /^https?:\/\/www\.ruanyifeng\.com\/blog\/.*$/g],
  [
    // https://www.toptal.com/laravel/restful-laravel-api-tutorial
    "toptal.js",
    /^https?:\/\/www\.toptal\.com\/.*$/g
  ],
  ["toutiao_io.js", /^https?:\/\/toutiao\.io\/?$/g],
  ["twitter.js", /^https?:\/\/twitter\.com($|\/.*)/g],
  ["zcfy_cc.js", /^https?:\/\/www\.zcfy\.cc\/article\/.*$/g],
  [
    "blog_miguelgrinberg.js",
    /^https?:\/\/blog\.miguelgrinberg\.com\/post\/.*?$/g
  ],
  ["codeship.js", /^https?:\/\/blog\.codeship\.com\/.*?$/g],
  ["jianshu.js", /^https?:\/\/www\.jianshu\.com\/p\/.*$/g],

  ["baidubaike.js", /^https?:\/\/baike\.baidu\.com\/item\/.*$/g],
  ["bo56.js", /^https?:\/\/www\.bo56\.com\/.+\/$/g],

  ["fourweekmba.js", /^https?:\/\/fourweekmba\.com\/.*$/g],
  [
    "cloud_tencent_community.js",
    /^https?:\/\/cloud\.tencent\.com\/community\/article\/\d+/g
  ],
  ["imhjm_article.js", /^http:\/\/imhjm\.com\/article\/.*$/g],
  [
    "csdn_blog.js",
    /^http:\/\/blog\.csdn\.net\/[^\/]+\/article\/details\/\d+$/g
  ],
  ["jiqizhixin.js", /^https:\/\/www\.jiqizhixin\.com\/articles\/[\d\-]+$/g],
  ["juejin.js", /^https?:\/\/juejin\.im\/post\/.*$/g],
  ["yiibai.js", /^https?:\/\/www.yiibai\.com\/.*$/g],
  ["ip_cn.js", /^https?:\/\/ip\.cn(\/.*$)?/g],
  [
  ...
  [
    ...
    /^https?:\/\/(www\.)?realpython\.com\/blog\/.*\/$/g
  ],
  ["", ""]
];
  ...
  for (var i = 0; i < content_scripts.length; i++) {
    ...
    if (content_script) {
      if (
        ...
        typeof patterns == "object" &&
        patterns.constructor.name == "Array"
      ) {
...
window.addEventListener(
  ...
  function(event) {
    ...
    console.log("window message:", event.source, event.data.text);
